...
<body>
	<div>
						...
						<div class="row">
							...
							<div class="col-12 col-sm-5 p-2" id="find-info-ksk">
								<div class="row p-2">
									...
									<div class="col-6 col-sm-12">
										<h3>${findboardDTO.address2}</h3>
									</div>
									<div class="p-1 col-6 col-sm-12">
										<i class="bi bi-tags-fill"></i>
										<c:choose>
											<c:when test="${findboardDTO.pet_type eq 0}">개</c:when>
											<c:when test="${findboardDTO.pet_type eq 1}">고양이</c:when>
											<c:when test="${findboardDTO.pet_type eq 2}">기타</c:when>
										</c:choose>
										/
										<c:choose>
										...
										/ ${findboardDTO.pet_age}살
									</div>
									<div class="p-1 col-6 col-sm-12">
										<i class="bi bi-calendar3"></i>
										<fmt:formatDate value="${findboardDTO.missing_date}"
											pattern="yyyy.MM.dd" />
									</div>
									<div class="p-1 col-6 col-sm-12">
										<i class="bi bi-geo-alt"></i> ${findboardDTO.detail_address}
										부근
									</div>
									<div class="p-1 col-6 col-sm-12">
										<i class="bi bi-coin"></i> 사례금 : <b>${findboardDTO.reward}</b>
										만원
									</div>
								</div>
							</div>
		...
		<div class="modal  modal-dialog-scrollable" id="find_content">
							...
							<div class="ModalSH">
								...
								<div class="modal-header">
									<h4 class="modal-title">
										<a><b>${findboardDTO.title}</b></a><a>(</a><a><b>${findboardDTO.address}</b>&nbsp;</a><a><b>${findboardDTO.address2}</b></a><a>)</a>
									</h4>
	...
	<script type="text/javascript">
		/* 		$(function(){
		 ...
		 $.ajax({
		 type: "GET",
		 //				data: JSON.stringify({"num":find_board_num}),
		 contentType : "application/x-www-form-urlencoded; charset=utf-8",
		 url:"${pageContext.request.contextPath }/ajaxfindcontent?find_board_num="+data,
		 //				dataType : "json"
		...
		/* 	$(document).on("click", ".openMod", function () { 
		 var boardnum = $(this).data('content'); 
		 $(".Modbody #content").val( boardnum );
		 )}; */
